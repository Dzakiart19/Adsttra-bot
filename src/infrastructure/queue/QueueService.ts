...
export class QueueService {
  ...
  public static initialize(redisUrl: string = process.env.REDIS_URL || 'redis://127.0.0.1:6379'): void {
    // Cek dulu apakah Redis bisa dijangkau — kalau tidak, skip setup Queue sepenuhnya
    // agar BullMQ tidak buat koneksi internal yang print ECONNREFUSED ke stderr.
    ...
    probe.on('error', () => {}); // silent

    probe.ping()
      .then(() => {
        probe.quit().catch(() => {});
        this._setupQueue(redisUrl);
      })
      .catch(() => {
        probe.quit().catch(() => {});
        logger.debug('QueueService: Redis tidak tersedia — mode lokal aktif');
        this.isInitialized = false;
      });
  }

  private static _setupQueue(redisUrl: string): void {
    try {
      ...
      this.redisConnection.on('error', (err) => {
        if (this.isInitialized) {
          logger.debug('QueueService: Redis error', { message: err.message });
          this.isInitialized = false;
        }
      });
      ...
      logger.info('QueueService initialized (Distributed mode aktif)', { redisUrl });
    } catch (err) {
      logger.warn('QueueService: Gagal setup Redis. Mode lokal aktif.');
      this.isInitialized = false;
    }
  }
